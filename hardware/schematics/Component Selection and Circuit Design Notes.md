...
#### Unipolar output
- Because the ADC cannot accept bipolar input, the TIA output  must be offset above ground. This is done by applying a 2.5V virtual ground to the non-inverting input of the TIA op-amp, which shifts the entire TIA output range up by 2.5V, keeping it within the safe input range of the ADC. The 2.5V reference 
is generated by a voltage divider on the 5V supply. The 2.5V offset is removed in firmware to recover the true signed current value.
...
### Power Supply
- In order to cover the ± 2V range, The opamps used in the control loop and the TIA, aswell as the switch used for gain switching need supply rails sufficient to produce the required output voltages with enough headroom
- A ±5V supply would be sufficient but leaves little margin, variation in supply voltage could result in clipping or distortion. applying a headroom of 2x gives a ± 9V minimum, the closest standard available supply is ±12V so it was selected for this project 
